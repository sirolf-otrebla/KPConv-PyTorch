...
class ModelNet40Dataset(PointCloudDataset):
    ...
    def __getitem__(self, idx_list):
        ...
        for p_i in idx_list:
            ...
            points, scale, R = self.augmentation_transform(points, normals)
            # Stack batch
            tp_list += [points]
            tn_list += [normals]
            tl_list += [label]
    ...
    def load_hdf5(self, filename):
        ...
        for line in open(filename, "r"):
            ...
            train_filenames.append(os.path.join(self.path, line))
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        print('\nLoading file {:s}'.format(filename))
        input_points, input_labels = self.load_hdf5(filename)
        self.num_models = input_labels.shape[0]
                # Number of models and models used per epoch
        if self.train:
            if self.config.epoch_steps and self.config.epoch_steps * self.config.batch_num < self.num_models:
                self.epoch_n = self.config.epoch_steps * self.config.batch_num
            else:
                self.epoch_n = self.num_models
        else:
            self.epoch_n = min(self.num_models, self.config.validation_size * self.config.batch_num)
        input_normals = None
